    ...
    for text in texts:
        if switcher[category](text, keyword):
            ...
            if category == 'description':
                start_idx = get_keyword_idx(text, keyword)
                # For category such as name, whitespace between words should be
                # kept
                return text[start_idx:].lstrip().rstrip()
...
def get_details(html):
    ...
    Input: html text containing the information
    Output: size, alcohol, city, country, brand, sugar, sweetness, style1, 
    style2, variety """
    texts = html.findAll(text=True)
    ...
    if madein_idx >= 0:
        # split by comma
        location = [x.strip() for x in texts[madein_idx].split(',')]
        if len(location) == 2:
    ...
    if style_idx >= 0:
        # split by ampersand
        styles = [x.strip() for x in texts[style_idx].split('&')]
        if len(styles) == 2:
    ...
    return size, alcohol, city, country, brand, sugar, sweetness, \
...
def check_avail_keyword(text, keyword):
    ...
    if len(keyword) == 1:
        ...
        # keyword = ['LCBO#:', 'VINTAGES#:']
        flags = [word in text for word in keyword]
        return any(flags)
...
def get_info_description(text, keyword):
    ...
    Output: boolean """
     ...
     # Basically pass new line strings and find the first actual words
    return len(text) > 5
...
def get_keyword_idx(text, keyword):
    ...
    Output: index """
    # Start index of whitespace after the keyword
    idx = len(text) - len(text.lstrip()) + len(keyword)

    return idx

def find_html_sections(soup):
    """ Find relevant sections for each feature extraction from the webpage 
    html script soup is mutable - i.e., passed by reference
    ...
    # Bottle Size, Alcohol/Vol, Made In, Brand, Sugar Content,
    # Sweetness Descriptor, Style, Varietal
    product_details_html = find_class(soup, 'product-details-list')
    
    # two recommended wines
    featured_wines_html = find_class(soup, 'recommendations-wrapper')
    # three recommended foods
    food_recomms_html = find_class(soup, 'carouselWrapper')
...
def scrape_page_raw(url):
    ...
    Example: soup = scrape_page_raw(url1)
    """
...
def format_data_df(values):
    """ Given a tuple of values, format it into dict so that it can be 
    appended to df
...
def scrape_product(url, index = -1):
    ...
    Input: url of the product to scrape, index of the product - used to save 
...
def parse_info(soup, url):
    ...
    Output: a tuple of scraped information
    Example: values = parse_info(soup)
    """
...
def read_saved_soup(file_name):
    ...
    Input: Beautifulsoup object to scrape, url to save into the df
    Output: a tuple of scraped information 
    Example: soup = read_saved_soup('html_source/0.html')
    """
